...
| 기능 | 요약 |
|---|---|
| 🗣️ AI 안부 상담원 | 페르소나 '보미'(단일 기준: `docs/bomi-persona.md`) — 첫 인사에서 AI임을 밝히고, 시간대별 선인사·짧은 존댓말·백채널 이해. 음성 대화(실시간 자막 미리보기 + CLOVA CSR 확정) |
| 📚 RAG 복지 검색 | **공식 자료 수백 건**(중앙부처 전국 + 대구·경북 지자체 + 수기 핵심 12)을 하이브리드 검색(FAISS 벡터+BM25 RRF). **어르신 적합성 필터**(수집·로드 이중 — 청년/근로자/산재/금융상품 배제). 근거 카드에 **출처·정보 기준일 + 복지로 링크(전 카드 보장)** |
...
구어체 평가셋(서비스명 없는 어르신 말투 12문 + 문서 밖 6문), **어르신 적합성 필터 적용 코퍼스(347청크)** 실 인덱스 기준:
...
```
거부 게이트: `top≥0.55 OR (top≥0.47 AND bm25≥11)` — 벡터 점수 분포가 겹쳐 단일 임계값이 불가능함을
실측으로 확인하고 2단으로 설계(근거: `app/rag/search.py` 주석, `scripts/eval_rag.py`).
⚠️ BM25 절대값은 코퍼스 크기에 민감 — 인덱스 재빌드 후엔 `eval_rag.py`로 임계 재확인.
...
공공데이터포털 2종 API ─┐                       어르신 발화 ─→ 게이트(벡터+BM25)
  중앙부처복지서비스     ├→ 어르신 필터(senior)  ──→ 통과: [복지 자료] 접지 → HCX-005 답변
  지자체(대구·경북 필터) ┘  → 복지카드 → 해시 증분      └→ 📌 T2 정보카드(코드 삽입·복지로 링크)
knowledge/welfare.json ──→ CLOVA 임베딩(bge-m3)      미달: 일반 수다(제도·수치 언급 금지)
                          → FAISS + BM25          판정 의도 → 슬롯 수집 → 룰엔진 → 📝 신청 패키지
   (로드 시에도 senior 필터 재적용 — 구 인덱스 방역)   (별도 비동기) HCX-007 특이사항 추출 + 안전망
                                                  → 패널/배너 + [상황 메모]로 다음 턴 대화에 환류
                                                  → 복지 니즈 감지 → 보미가 제안 → "응" → 근거 안내
```
